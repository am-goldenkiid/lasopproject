...
function Home() {
  ...
  useEffect(() => {
   axios.get(`${process.env.REACT_APP_API_URL}/students`).then(res => (setTotalStudents(res.data.result[0].qty)))
   axios.get(`${process.env.REACT_APP_API_URL}/courses`).then(res => (setTotalCourses(res.data.result[0].qty)))
  }, []);

  const datas = [
    { title: "No Of Students", total:  totalStudents , icon: profAdd },
    { title: "No Of Staffs", total: 12, icon: teacher },
    { title: "No Of centers", total: 8, icon: house },
    { title: "No Of Courses", total: totalCourses, icon: bkmark },
    { title: "Current Cohorts", total: 6, icon: chrt },
    { title: "Completed Cohorts", total: 20, icon: comp },
    { title: "New Applicant", total: totalStudents, icon: stud },
